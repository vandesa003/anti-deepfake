...
logger = init_logging(log_dir=os.path.abspath("../logs/"), log_file="frame_process.log")
...
if __name__ == "__main__":
    ...
    for folder in folder_list:
        print('start to process folder', folder)
        logger.info("start to process folder: {}".format(folder))
        file_path = os.path.join(path, folder)
        video_files = [file for file in glob(os.path.join(file_path, "*.mp4"))]
