...
def predict_on_video(video_path, batch_size):
    # input image size for Xception Net.
    input_size = 300
    ...
    faces, probs = get_face_patches(frames, margin=10)
    final_faces = []
    for i in faces:
        if i is not None:
            final_faces.append(i)
    final_probs = []
    for i in probs:
        if i is not None:
            final_probs.append(i)
    face_num = len(final_faces)
    print(face_num)
    if face_num > 0:
        faces_tensor = torch.stack(final_faces)
        print(faces_tensor.shape)
        # NOTE: When running on the CPU, the batch size must be fixed
        # or else memory usage will blow up. (Bug in PyTorch?)
        resized_face = F.interpolate(faces_tensor, size=input_size)
        # If we found any faces, prepare them for the model.

        tmpImg = np.zeros((face_num, input_size, input_size, 3))
        resized_face = resized_face.permute(0, 2, 3, 1)
        print(resized_face.shape)
        # Convert image from [0, 255] to [0, 1].
        image = resized_face / torch.max(resized_face)
        if image.shape[3] == 1:
        ...
        x = torch.tensor(tmpImg, device=torch.device("cuda")).float()
        x = x.permute(0, 3, 1, 2)
        ...
        with torch.no_grad():
            ...
            return y_pred[:face_num].mean().item()

    return 0.5


if __name__ == "__main__":
    model = BinaryXception(2048)
    model.load_state_dict(torch.load("../saved_models/model.pth"))
    model.cuda()
    filename = []
    label = []
    for vid in tqdm(glob("../dataset/dfdc_train_part_0/*.mp4")):
        score = predict_on_video(vid, 20)
        filename.append(vid.split("/")[-1])
        label.append(score)
        print(score)
